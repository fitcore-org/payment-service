...
class PlanController(private val planService: PlanService) {
    ...
    fun deletePlan(
        @PathVariable planId: String,
    ) = planService.deletePlan(planId).let { ResponseEntity.noContent().build<Void>() }

    @GetMapping
    fun listPlans() = ResponseEntity.ok(planService.listPlans().map { PlanMapper.toResponse(it) })
}
