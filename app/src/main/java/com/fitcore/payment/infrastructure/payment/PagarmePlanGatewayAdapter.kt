...
/**
 * Adapter for integrating with Pagar.me's Plan API endpoints.
 * Handles plan and plan item operations via HTTP requests.
 */
@Component
class PagarmePlanGatewayAdapter(
    ...
    /**
     * Parses a plan response from Pagar.me API into the domain Plan model.
     * @param resp Response map from Pagar.me.
    ...
    private fun parsePlanFromResponse(resp: Map<String, Any?>): Plan =
        Plan(
            ...
            items = (resp["items"] as? List<*>)?.mapNotNull { item ->
                if (item is Map<*, *>) {
                    PlanItem(
            ...
            pricingScheme = (resp["pricing_scheme"] as? Map<*, *>)?.let { scheme ->
                PricingScheme(
                    schemeType = scheme["scheme_type"]?.toString(),
                    price = (scheme["price"] as? Number)?.toInt(),
                )
    ...
    /**
     * Creates a new plan.
     * @param plan The Plan domain model to create.
    ...
    override fun createPlan(plan: Plan): Plan {
        ...
        val body = mutableMapOf<String, Any?>(
            ...
            "items" to plan.items?.map {
                mutableMapOf<String, Any?>(
                    "name" to it.name,
                    "quantity" to it.quantity,
                    "pricing_scheme" to mapOf(
            ...
            "pricing_scheme" to plan.pricingScheme?.let {
                mapOf(
     ...
     * @param plan The Plan domain model with new data.
     ...
     * @param item The PlanItem domain model.
    ...
    /**
     * Deletes an item from a plan by its ID.
     * @param planId The ID of the plan.
