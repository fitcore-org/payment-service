...
class PagarmePlanGatewayAdapter(
    ...
    private fun parsePlanFromResponse(resp: Map<String, Any?>): Plan = Plan(
        id = resp["id"]?.toString(),
        ...
        items = (resp["items"] as? List<*>)?.mapNotNull { item ->
            if (item is Map<*, *>) {
                PlanItem(
                    ...
                    pricingScheme = (item["pricing_scheme"] as? Map<*, *>)?.let { scheme ->
                        PricingScheme(
    ...
    override fun updatePlan(planId: String, plan: Plan): Plan {
        val url = "$baseUrl/plans/$planId"
        val headers = buildHeaders()
        val body = mutableMapOf<String, Any?>(
            "name" to plan.name,
    ...
    override fun addPlanItem(planId: String, item: PlanItem): PlanItem {
    ...
    override fun updatePlanItem(planId: String, itemId: String, item: PlanItem): PlanItem {
    ...
    override fun deletePlanItem(planId: String, itemId: String) {
        val url = "$baseUrl/plans/$planId/items/$itemId"
        val headers = buildHeaders()
        restTemplate.exchange(url, HttpMethod.DELETE, HttpEntity(null, headers), Void::class.java)
    }
